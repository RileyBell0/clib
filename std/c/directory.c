...
alist_t dir_all_entries_alist(string_t *path) {
  ...
  // The file name probably wont be too long so no need to pre-allocate memory
  // for it
  ...
  while ((entry = readdir(d))) {
    if (entry->d_type == DT_UNKNOWN) {
      ...
      string_write_c(&filePath, entry->d_name);
...
alist_t dir_all_entries_of_type(string_t *path, unsigned char type) {
   ...
   * its faster than calling the above then filtering so in this case we're re-writing
   * code
   */
  DIR *d = opendir(cstr(path));
  alist_t entries = new_alist(sizeof(struct dirent));
  ...
  struct dirent *entry;
  string_t file_path = string_make(path);
  string_t entry_name = new_string(SMALL_BUFFER_LEN);
  uint32_t path_len = file_path.len;

  while ((entry = readdir(d))) {

    if (entry->d_type == DT_UNKNOWN) {
      // Reset the strings for the new 
      string_shrink(&file_path, path_len);
      string_clear(&entry_name);

      // Try harder to get the file's type
      string_write_c(&entry_name, entry->d_name);
      string_write(&file_path, &entry_name);
      entry->d_type = get_file_type(cstr(&file_path));
    }
...
categorised_dirent_t dir_all_entries_categorised(string_t *path) {
  categorised_dirent_t ordered = new_categorised_dirent_t();
  ...
  while ((entry = readdir(d))) {
    ...
    categorised_dirent_insert(&ordered, entry);
  }
  string_destroy(&filePath);
 ...
 * like for instance we store the starting path of each string related to each
 * matching file path this is wasted space, and thus wasted performance.
 *
 * instead we could create our own tree-like structure to act as a directory
 * which we can filter to only have stuff like files with the matching
 * directory, then we can make an iterator which could keep within it the
 * current node so im thinking we make a tree structure, each node has the name
 * of itself (the directory) and it has a list of files, and these only have
 * thier file names in them
 *
 * then the iterator can keep track of the list it's in and the path up to that
 * point. so it would start at the root, list all the files in that directory
 * (while also hvaing it's path attached to the file if requested) and then when
 * you call next it'll go to the next node in the tree, put its name in the plae
 * of the last file and return that string
 *
 ...
 * }
 *
 * idk, i want a function which remembers its state (through using a iterator
 * struct) which can be used to progressively traverse a tree at each call. This
 * might not be easy, but it shouldn be too hard
 *
 * itll need to know where it is in the tree, what it's looking at and how to
 * get to the next element from what it's looking at and from there it should be
 * a piece of cake
 *
 * the only problem is that this iterator might have to allocate memory
 * so we're going to have to see how much we can get away with without
 * allocating memory because fuck that and idk what im doing but im sure i can
 * figure it out
 *
...
alist_t dir_files_with_extension_recur(string_t *path, string_t *extension) {
  ...
  // categorised_dirent_t entries = dir_all_entries_categorised(path);
  ...
  // while (node)
  // {
  //     // restrict the path back to the current directory (the path recieved
  //     by this function) cstr(&subDirPath)[path.len] = '\0';
  ...
  //         alist_t subDirFiles = dir_files_with_extension_recur(subDirPath,
  //         extension); list_combine(&matchingFiles, &subDirFiles);
  //     }
  //     node = node->next;
  // }
...
string_t *lstrnode(list_node_t *node) { return (string_t *)node->data; }