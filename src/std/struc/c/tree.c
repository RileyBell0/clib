...
tree_t tree_new(size_t elem_size,
  ...
  tree.block_size = sizeof(tree_node_t) + elem_size;
  tree.size = 0;
  tree.head = NULL;
...
// Requires link_to_start to be valid and start to be valid.
tree_node_t *tree_node_pop_max(tree_node_t **link_to_start,
  ...
  while (true) {
    if (start->right == NULL) {
      if (!link_to_start) {
        // TODO this should throw an error for invalid function usage
        return NULL;
      }
...
tree_node_t *tree_node_pop_min(tree_node_t **link_to_start,
  ...
  while (true) {
    if (start->left == NULL) {
      if (!link_to_start) {
        for (int i = 0; i < 100; i++) {
          printf("HELOP HELOP HELOP HEKOPHEKOPEHJPOHEROPH\n");
        }
        // TODO this should throw an error for invalid function usage
        return NULL;
      }
