 ...
 *      -> O(1)
 *         good average time complexity O(1), but significantly the space where
 * the new element is going to be inserted is already allocated alot of the time
 * Deletion: Really fast
 *      -> O(1)
 *         there is no need to free the memory due to the structure the data
 * being in an array there is generally (especially in smaller lists) huge
 * advantages in memory locality due to all elements being in similar places in
 * memory due to being stored in the same chunk however deleting an element has
 * an added cost in comparison to a linked-list as the deleted element needs to
 * have the last element in the array copied into its place and the referenes
 * for that element fixed aswell to point at its new position
 *
 *         so deleition is still fast, and benefits from memory locality but it
 * has more to do in a deletion Search: Quite fast
 *      -> This is where the memory locality helps out, all list elements are
 * stored together so we get the added boost of hvaing less cache misses because
 * especially in smaller lists, the entire list will be able to be stored in the
 * cache, or at least theres a high likely hood that at least some of the next
 * elements will be in the cache
 *
 *         i dont actually know how cache works
 *
 * Drawbacks:
 *      ->  No element's position in memory is guaranteed using this data
 * structure the list is faster with smaller element sizes due to stuff being
 * frequently moved around within the list
 *
 *          so no element's position in memory is guaranteed, and its position
 * relative to the list's origin in memory is also not guaranteed.
 *
 *          it's faster at inserting stuff than a list, assuming faster at
 * searching stuff than a list and when an atree_t structure exists it'll be
 * faster for doing anything sorted-related than using a list or a tree
 *
 * from testing, insertion is magnitudes of times faster than in a linked list
 * when all optimisations are turned on in the compiler. but worth noting theres
 * a larger range of performance than when comparing to an array
 */
...
void *list_remove_at(list_t *list, unsigned int index) {
  ...
  // Through the use of dark magic, the node at the given index could not be
  // found
  return NULL;
}
...
list_t *list_combine(list_t *base, list_t *extension) {
  ...
  // Encouraging the person using this function to not use the 'extension' list
  // again
