...
static avl_bin_tree_node_t *avl_bin_tree_new_node(avl_bin_tree_t *tree,
                                                  void *data);
static avl_bin_tree_node_t *avl_bin_tree_insert_node(avl_bin_tree_t *tree,
                                                     avl_bin_tree_node_t *node,
                                                     void *data);
static void avl_bin_tree_destroy_all_nodes(avl_bin_tree_node_t *treeNode,
...
// returns a new avl_bin_tree_node_t struct with its first element in its new
// list set to 'data'
static avl_bin_tree_node_t *avl_bin_tree_new_node(avl_bin_tree_t *tree,
                                                  void *data) {
  avl_bin_tree_node_t *node =
      (avl_bin_tree_node_t *)safe_calloc(sizeof(avl_bin_tree_node_t));
...
avl_bin_tree_node_t *avl_bin_tree_insert_node(avl_bin_tree_t *tree,
                                              avl_bin_tree_node_t *node,
                                              void *data) {
...
static avl_bin_tree_node_t *avl_rotate_left(avl_bin_tree_node_t *x) {
  ...
  //  Update heights
  x->height = max_int(avl_bin_tree_node_height(x->left),
                      avl_bin_tree_node_height(x->right)) +
              1;
  y->height = max_int(avl_bin_tree_node_height(y->left),
                      avl_bin_tree_node_height(y->right)) +
              1;
...
void avl_bin_tree_destroy(avl_bin_tree_t tree,
                          void (*delete_data)(void *data)) {
  avl_bin_tree_destroy_all_nodes(tree.root, delete_data);
}

// destroys a tree node, calling the delete_data function on all of its stored
// data
static void avl_bin_tree_destroy_all_nodes(avl_bin_tree_node_t *treeNode,
                                           void (*delete_data)(void *data)) {
  if (treeNode != NULL) {
...
void avl_bin_tree_fill_arr(array_t *arr, avl_bin_tree_node_t *node, int *pos,
